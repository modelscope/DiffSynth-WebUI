...
WEBUI_ROOT="$(cd "$TRAINING_UI_DIR/.." && pwd)"
export DIFFSYNTH_STUDIO_ROOT="${DIFFSYNTH_STUDIO_ROOT:-$WEBUI_ROOT/DiffSynth-Studio}"
if [ ! -d "$DIFFSYNTH_STUDIO_ROOT/diffsynth" ] || [ ! -d "$DIFFSYNTH_STUDIO_ROOT/examples" ]; then
  echo "[launch] invalid DiffSynth-Studio root: $DIFFSYNTH_STUDIO_ROOT" >&2
  echo "[launch] initialize the submodule or set DIFFSYNTH_STUDIO_ROOT." >&2
  exit 1
fi
export PYTHONPATH="$DIFFSYNTH_STUDIO_ROOT:$TRAINING_UI_DIR${PYTHONPATH:+:$PYTHONPATH}"
...
export LOG_DIR="${DIFFSYNTH_UI_LOG_DIR:-/tmp/diffsynth-webui-${NEXT_PORT}}"
  ...
  (
    if [ "$VERBOSE" = "1" ]; then
      exec env BASE_PATH="$NEXT_BASE_PATH" LISTEN_PORT="$NEXT_PORT" \
        UPSTREAM_PORT="$UPSTREAM_PORT" BACKEND_PORT="$BACKEND_PORT" node "$ADAPTER_PATH"
    else
      exec env BASE_PATH="$NEXT_BASE_PATH" LISTEN_PORT="$NEXT_PORT" \
        UPSTREAM_PORT="$UPSTREAM_PORT" BACKEND_PORT="$BACKEND_PORT" QUIET=1 node "$ADAPTER_PATH" \
