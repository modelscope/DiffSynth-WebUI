...
export function GpuMonitor() {
  ...
  useEffect(() => {
    ...
    async function reload() {
      try {
        ...
        if (alive) setError(err?.message || "Unable to read GPU status");
      }
    }
    reload();
  ...
  return (
    <Card title="GPU Resources" className="h-full">
      {gpus.length === 0 ? (
        <div className="py-6 text-center text-sm text-slate-400">No GPU detected</div>
      ) : (
      ...
      {error && (
        <div className="mt-3 rounded-md border border-red-500/25 bg-red-500/5 px-3 py-2 text-xs text-red-300">
          GPU status is currently unavailable: {error}
        </div>
      )}
    </Card>
  );
