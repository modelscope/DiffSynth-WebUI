...
const NAV = [
  ...
  { label: "Training Datasets", href: "/datasets", icon: "folder" },
...
function isNavActive(pathname: string, href: string) {
  ...
  if (href === "/tasks/new") return pathname.startsWith("/tasks/new/");
    ...
    <aside className="flex h-full w-60 shrink-0 flex-col border-r border-slate-800 bg-slate-950 p-4">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-base font-semibold leading-tight text-white">DiffSynth-WebUI</div>
          </div>
        </div>
      </div>
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={
                "flex items-center gap-2.5 rounded-md border px-3 py-2 text-base transition-colors " +
                (active
