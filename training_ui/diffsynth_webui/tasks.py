...
class TaskRecord:
    ...
    created_at: str
    updated_at: str
    latest_run: Optional[TaskRunRecord] = None
...
def _row_to_run(row) -> TaskRunRecord:
    return TaskRunRecord(
        id=row["id"], task_id=row["task_id"], task_name=row["task_name"],
...
def _latest_run_with_conn(conn, task_id: str) -> Optional[TaskRunRecord]:
    row = conn.execute(
...
def _row_to_task(conn, row) -> TaskRecord:
...
def create_task(name: str, model_type: str, dataset: str, config_data: Dict[str, Any]) -> TaskRecord:
    settings.apply_path_settings()
    task_id = uuid.uuid4().hex[:12]
    task_dir = config.OUTPUTS_ROOT / f"{task_id}_{_safe_task_name(name)}"
    task_dir.mkdir(parents=True, exist_ok=False)
    now = _now()
    try:
        with db.tasks_conn() as conn:
            conn.execute(
                """
                INSERT INTO tasks (
                ...
                """,
                (task_id, name, model_type, dataset, json.dumps(config_data, ensure_ascii=False), now, now, str(task_dir)),
            )
    except Exception:
...
def update_task(task_id: str, **fields: Any) -> None:
    ...
    if not fields or not set(fields).issubset(allowed):
        ...
        if invalid:
            raise ValueError(f"run fields cannot be stored on tasks: {sorted(invalid)}")
        return
    columns = ", ".join(f"{key} = ?" for key in fields)
    with db.tasks_conn() as conn:
        conn.execute(f"UPDATE tasks SET {columns} WHERE id = ?", [*fields.values(), task_id])


def edit_task(task_id: str, name: str, model_type: str, dataset: str, config_data: Dict[str, Any]) -> TaskRecord:
    task = get_task(task_id)
    if name != task.name:
        raise ValueError("The task name cannot be changed after creation")
    if task.latest_run and task.latest_run.status in ACTIVE_RUN_STATUSES:
        raise ValueError("A running task cannot be edited. Stop it first.")
    update_task(
        task_id, model_type=model_type, dataset=dataset,
        config_json=json.dumps(config_data, ensure_ascii=False), updated_at=_now(),
    )
    return get_task(task_id)


def create_run(task_id: str) -> TaskRunRecord:
    task = get_task(task_id)
    if task.latest_run and task.latest_run.status in ACTIVE_RUN_STATUSES:
        raise ValueError("The task already has a preparing or running process")
        ...
        with db.tasks_conn() as conn:
            conn.execute(
                ...
                INSERT INTO task_runs (
                    id, task_id, task_name, status, config_json, output_path, created_at
                ) VALUES (?, ?, ?, 'preparing', ?, ?, ?)
                """,
                (run_id, task.id, task.name, task.config_json, str(output_path), now),
            )
    except Exception:
...
def get_run(run_id: str) -> TaskRunRecord:
    with db.tasks_conn() as conn:
...
def latest_run(task_id: str) -> Optional[TaskRunRecord]:
    with db.tasks_conn() as conn:
        return _latest_run_with_conn(conn, task_id)


def list_runs(task_id: str) -> List[TaskRunRecord]:
    with db.tasks_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM task_runs WHERE task_id = ? ORDER BY created_at DESC, rowid DESC",
            (task_id,),
        ).fetchall()
...
def update_run(run_id: str, **fields: Any) -> None:
    ...
    columns = ", ".join(f"{key} = ?" for key in fields)
    with db.tasks_conn() as conn:
        conn.execute(f"UPDATE task_runs SET {columns} WHERE id = ?", [*fields.values(), run_id])


def delete_task_records(task_id: str) -> None:
    with db.tasks_conn() as conn:
