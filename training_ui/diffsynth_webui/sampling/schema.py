...
class SampleRequest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SampleRequest":
        ...
        if not isinstance(inputs, dict):
            raise ValueError("sampling inputs must be a JSON object")
        return cls(prompt=prompt, inputs=dict(inputs), output=output)
...
class SamplingConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SamplingConfig":
        required = ("run_id", "model_type", "checkpoint", "output_dir")
        ...
        if not isinstance(pipeline, dict) or not pipeline.get("module") or not pipeline.get("class"):
            raise ValueError("sampling config is missing a direct pipeline specification")
        input_schema = data.get("input_schema") or []
        if not isinstance(input_schema, list):
            raise ValueError("sampling input_schema must be a JSON array")
        return cls(
            **values, pipeline=dict(pipeline), input_schema=list(input_schema),
