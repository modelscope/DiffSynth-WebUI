...
from . import config, datasets, tasks, recipes, settings
...
def _resolve_extra_inputs(dataset_name: str) -> str:
    if not dataset_name:
        return ""
    return ",".join(datasets.get_extra_input_keys(dataset_name))
...
def build_command(
    ...
    remove_prefix = recipe.remove_prefix
    ...
    if resume_from_checkpoint:
        ...
        add("resume_from_checkpoint", str(checkpoint_path.resolve()))
    ...
    argv.append("--enable_csv_log")
    if recipe.default_use_gradient_checkpointing:
        argv.append("--use_gradient_checkpointing")

    fixed = recipe.fixed_training_args
    extra = _resolve_extra_inputs(ds_name)
    if extra:
        add("extra_inputs", extra)

    add("gradient_accumulation_steps", fixed.get("gradient_accumulation", 1))
    add("dataset_num_workers", fixed.get("dataset_num_workers", 0))
    if fixed.get("find_unused_parameters"):
    ...
    if len(recipe.training_stages) > 1:
        if resume_from_checkpoint:
            raise ValueError("resume_from_checkpoint is only supported for single-stage training")
        argv = _build_staged_command(argv, recipe, ds_dir, metadata_path, Path(output_path), cfg)

    return argv, str(output_path), str(log_path)


def prepare_run(run_id: str) -> tasks.TaskRunRecord:
    run = tasks.get_run(run_id)
    cfg = dict(run.config)
    cfg.setdefault("task_name", run.task_name)
    recipe = recipes.get_recipe(cfg["model_type"])
    ...
    return tasks.get_run(run.id)


def _read_prepared_training_config(run: tasks.TaskRunRecord) -> Dict[str, Any]:
    ...
    if payload.get("task_id") != run.task_id or payload.get("run_id") != run.id:
...
def _build_staged_command(
    ...
    # Rebuild each stage from one base command so stage-specific flags cannot leak.
    ...
    data_process_then_train = (
        len(recipe.training_stages) == 2
        and str(recipe.training_stages[0].get("task", "")).endswith(":data_process")
        and str(recipe.training_stages[1].get("task", "")).endswith(":train")
    )
    stage_specific = {
    ...
    for index, stage in enumerate(recipe.training_stages):
        ...
        if "num_epochs" in configured_stage and configured_stage["num_epochs"] is not None:
            value = int(configured_stage["num_epochs"])
            if value <= 0:
                raise ValueError(f"num_epochs in stage {index + 1} must be a positive integer")
            stage["num_epochs"] = value
        if data_process_then_train:
            stage["num_epochs"] = 1 if index == 0 else cfg.get("num_epochs", recipe.default_epochs)
        for key in ("max_timestep_boundary", "min_timestep_boundary"):
            if key in stage and configured_stage.get(key) is not None:
                ...
                if not math.isfinite(value) or not 0 <= value <= 1:
                    raise ValueError(f"{key} in stage {index + 1} must be between 0 and 1")
                stage[key] = str(value)
        if "num_epochs" not in stage:
            stage["num_epochs"] = (
                cfg.get("num_epochs", recipe.default_epochs)
                if str(stage.get("task", "")).endswith(":train")
                else 1
            )
        ...
        if minimum is not None and maximum is not None and float(minimum) > float(maximum):
            raise ValueError(f"min_timestep_boundary cannot exceed max_timestep_boundary in stage {index + 1}")
...
def start_task(task_id: str) -> tasks.TaskRecord:
    task = tasks.get_task(task_id)
    if task.latest_run and task.latest_run.status in tasks.ACTIVE_RUN_STATUSES:
        return task
    run = tasks.create_run(task_id)
    try:
        run = prepare_run(run.id)
        payload = _read_prepared_training_config(run)
    except Exception:
        tasks.update_run(run.id, status="failed", finished_at=_now())
        raise
    ...
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "diffsynth_webui.run_worker", "--run-id", run.id],
...
def stop_task(task_id: str) -> None:
    ...
    if not run:
        return
    if not run.os_pid:
        tasks.update_run(run.id, status="stopped", finished_at=_now())
        return
    try:
    ...
    if run.status == "sampling":
        fields.update({
            ...
            "sampling_message": "Sampling was stopped by the user.",
            "sampling_finished_at": _now(),
        })
    tasks.update_run(run.id, **fields)
...
def refresh_status(task_id: str) -> tasks.TaskRecord:
    task = tasks.get_task(task_id)
    run = task.latest_run
    if not run or run.status not in tasks.ACTIVE_RUN_STATUSES:
        return task
    ...
    tasks.update_run(run.id, status=new_status, returncode=rc, finished_at=_now())
    if run.status == "sampling" and rc == 0:
        tasks.update_run(
            run.id,
            sampling_status="failed",
            sampling_message="The sampling supervisor exited unexpectedly.",
            sampling_finished_at=_now(),
        )
    return tasks.get_task(task_id)
